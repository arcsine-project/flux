...
struct [[nodiscard]] advance_fn final {
    ...
    constexpr void operator()(I& i, meta::iter_diff_t<I> n) const noexcept {
    ...
    constexpr void operator()(I& i, Sentinel bound) const noexcept {
        // If `I` and `S` model `assignable_from<I&, S>`, equivalent to `i = std::move(bound)`.
        if constexpr (meta::assignable_from<I&, Sentinel>) {
            i = static_cast<Sentinel&&>(bound);
        } else if constexpr (meta::sized_sentinel_for<Sentinel, I>) {
    ...
    template <meta::input_or_output_iterator I, meta::sentinel_for<I> Sentinel>
    constexpr auto operator()(I& i, meta::iter_diff_t<I> n, Sentinel bound) const noexcept {
        // If `S` and `I` model `sized_sentinel_for<S, I>`:
        if constexpr (meta::sized_sentinel_for<Sentinel, I>) {
            ...
            // times.
            if constexpr (meta::bidirectional_iterator<I> && meta::same_as<I, Sentinel>) {
                for (; n < 0 && i != bound; ++n) {
                    --i;
                }
            } else {
