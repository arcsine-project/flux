...
void check_move(MemoryList& list) {
    ...
    MemoryList list2(::std::move(list));
    CHECK(list.empty());
    CHECK(list.capacity() == 0u);
    CHECK(!list2.empty());
...
void use_list_array(detail::free_list& list) {
    ::std::vector<void*> ptrs;
    auto                 capacity = list.capacity();
...
TEST_CASE("fou::detail::unordered_free_list", "[flux-memory/unordered_free_list.hpp]") {
    SECTION("construct") {
        detail::unordered_free_list list(4);
        REQUIRE(list.empty());
    ...
    SECTION("move") {
        ...
        detail::unordered_free_list    new_list(::std::move(list));
        check_list(new_list, &new_memory, 1024);
    }
}

TEST_CASE("fou::detail::free_list", "[flux-memory/unordered_free_list.hpp]") {
    SECTION("construct") {
    ...
    SECTION("move") {
        ...
        static_allocator_storage<1024> new_memory;
        detail::free_list              new_list(::std::move(list));
        check_list(new_list, &new_memory, 1024);
