...
// It uses a `memory_arena` with a given `BlockOrRawAllocator` defaulted to
// `growing_block_allocator`, subdivides them in small nodes of given size and puts them onto a free
// list. Allocation and deallocation simply remove or add nodes from this list and are thus fast.
// The way the list is maintained can be controlled via the `PoolType` which is either `node_pool`
// or `array_pool`. This kind of allocator is ideal for fixed size allocations and deallocations in
// any order, for example in a node based container like `::std::list`. It is not so good for
// different allocation sizes and has some drawbacks for `array`s.
// clang-format off
template <
...
private:
    constexpr auto info() const noexcept {
        return allocator_info{"flux::fou::memory_pool", this};
    }
...
struct [[nodiscard]] composable_traits<memory_pool<PoolType, BlockOrRawAllocator>> final {
                      ...
                      size_type       alignment) noexcept
    {
        ...
        constexpr bool invalid_node_size  = size      > allocator_traits::max_node_size(allocator);
        constexpr bool invalid_align      = alignment > allocator_traits::max_alignment(allocator);
        if (invalid_node_size || invalid_align)
            return nullptr;
                       ...
                       size_type       alignment) noexcept
    {
        ...
        constexpr bool invalid_node_size  = size         > allocator_traits::max_node_size(allocator);
        constexpr bool invalid_array_size = count * size > allocator_traits::max_array_size(allocator);
        constexpr bool invalid_align      = alignment    > allocator_traits::max_alignment(allocator);
        if (invalid_node_size || invalid_array_size || invalid_align)
            return nullptr;
