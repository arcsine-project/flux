...
struct stub_allocator {
    ...
    void* allocate_node(size_type, size_type) noexcept {
        return nullptr;
    }
    void deallocate_node(void*, size_type, size_type) noexcept {}

    void* allocate_array(size_type, size_type, size_type) noexcept {
        return nullptr;
    }
    void deallocate_array(void*, size_type, size_type, size_type) noexcept {}

    void* try_allocate_node(size_type, size_type) noexcept {
        return nullptr;
    }
    bool try_deallocate_node(void*, size_type, size_type) noexcept {
        return false;
    }

    void* try_allocate_array(size_type, size_type, size_type) noexcept {
        return nullptr;
    }
    bool try_deallocate_array(void*, size_type, size_type, size_type) noexcept {
        return false;
    }
};

template <typename Allocator>
void check_allocate_node(flux::fou::allocator_reference<Allocator> ref) {
    auto const size  = sizeof(int);
    auto const align = alignof(int);

    auto* memory = ref.allocate_node(size, align);
    CHECK(memory);
    CHECK(flux::fou::is_aligned(memory, align));
    ref.deallocate_node(memory, size, align);
}

template <typename Allocator>
void check_allocate_array(flux::fou::allocator_reference<Allocator> ref) {
...
TEST_CASE("fou::allocator_adapter", "[flux-memory/allocator_storage.hpp]") {
    ...
    fou::heap_allocator                         allocator;
    fou::allocator_adapter<fou::heap_allocator> adapter(::std::move(allocator));
    CHECK_FALSE(adapter.is_composable());
    check_allocate_node<fou::heap_allocator>(adapter);

    fou::allocator_adapter<fou::heap_allocator> new_adapter(::std::move(adapter));
    check_allocate_array<fou::heap_allocator>(new_adapter);

    fou::allocator_adapter<fou::heap_allocator> other_adapter;
    other_adapter = ::std::move(new_adapter);
    check_allocate_node<fou::heap_allocator>(other_adapter);
}

TEST_CASE("fou::allocator_storage", "[flux-memory/allocator_storage.hpp]") {
    using namespace flux;
    using test_allocator = fou::test_allocator<::std::unordered_map>;

    SECTION("test stateful reference") {
        test_allocator                           allocator;
        fou::allocator_reference<test_allocator> ref_stateful(allocator);
        ...
        CHECK(ref_stateful.max_alignment() == fou::detail::max_alignment);
    ...
    SECTION("test stateless reference") {
        fou::heap_allocator                           allocator;
        fou::allocator_reference<fou::heap_allocator> ref_stateless(allocator);
        ...
        CHECK(ref_stateless.max_alignment() == fou::detail::max_alignment);
    ...
    SECTION("test any allocator reference") {
        ...
        fou::any_allocator_reference any_ref(allocator);
        ...
        CHECK(any_ref.max_alignment() == fou::detail::max_alignment);
    ...
    SECTION("test composable allocator reference") {
        ...
        fou::allocator_reference<stub_allocator> ref(allocator);
    ...
    SECTION("test composable any allocator reference") {
        ...
        fou::any_allocator_reference ref(allocator);
        CHECK(ref.is_composable());
        ...
        fou::any_allocator_reference other_ref(allocator);
        other_ref = ref;
    ...
    SECTION("test get stored allocator") {
        test_allocator                           allocator;
        fou::allocator_reference<test_allocator> ref(allocator);
        ...
        fou::allocator_reference<test_allocator> const ref_const(allocator);
    ...
    SECTION("test move/copy") {
        ...
        fou::allocator_reference<test_allocator> new_ref(::std::move(ref));
        check_allocate_array(new_ref);

        fou::allocator_reference<test_allocator> other_ref(allocator);
    ...
    SECTION("test shared reference") {
        ...
        using reference_shared = fou::detail::reference_shared;
        struct shared_storage final
                : fou::detail::reference_storage_base<stub_allocator, reference_shared> {
            using base = fou::detail::reference_storage_base<stub_allocator, reference_shared>;
