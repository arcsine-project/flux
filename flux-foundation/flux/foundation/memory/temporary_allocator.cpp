...
#if FLUX_MEMORY_TEMPORARY_STACK_MODE >= 2
// NOTE:
...
static struct [[nodiscard]] temporary_list {
    ...
    void destroy() noexcept {
        ...
        FLUX_ASSERT(!first.load(), "destroy() called while other threads are still running");
    }
...
#elif FLUX_MEMORY_TEMPORARY_STACK_MODE == 1
// NOTE:
...
temporary_stack& stack(::std::size_t) noexcept {
    FLUX_ASSERT("temporary_stack(std::size_t) called but stack is disabled by "
                "FLUX_MEMORY_TEMPORARY_STACK_MODE == 0");
    fast_terminate();
}
#endif
...
temporary_allocator::temporary_allocator(temporary_stack& stack) noexcept
    ...
    FLUX_ASSERT(!prev_ || prev_->is_active());
    stack.top_ = this;
}
...
bool temporary_allocator::is_active() const noexcept {
    FLUX_ASSERT(unwinder_.will_unwind());
    bool const active = unwinder_.stack().top_ == this;
    // check that prev is actually before this
    FLUX_ASSERT(!active || !prev_ || prev_->unwinder_.marker() <= unwinder_.marker());
    return active;
}
